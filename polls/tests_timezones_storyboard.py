...
@tag("browser_storyboard", "timezone_contract")
@override_settings(ALLOWED_HOSTS=["127.0.0.1", "localhost", "testserver", "[::1]"])
class PollTimezoneContractStoryboardTests(StaticLiveServerTestCase):
    ...
    def tab_until(
        ...
        *,
        ...
        for _ in range(limit):
            ...
            matches_text = text is None or snapshot["text"] == text
            matches_id = element_id is None or snapshot["id"] == element_id
            matches_option_id = option_id is None or snapshot["optionId"] == option_id
            matches_status = status is None or snapshot["status"] == status
            if matches_text and matches_id and matches_option_id and matches_status:
                return
